...
describe('Electron onboarding boundary', () => {
  ...
  it('maps all important recovery and security failures to explicit view states', () => {
    assert.equal(errorCategory('ONBOARDING_ENDPOINT_PRIVATE_REVIEW_REQUIRED'), 'private-review-required')
  ...
  it('provides keyboard semantics, non-color status text, CSP isolation, and responsive overflow guardrails', async () => {
    ...
    assert.match(html, /id="mode-quick"[^>]+tabindex="0"/)
    assert.match(html, /id="mode-full"[^>]+tabindex="-1"/)
    assert.match(html, /id="detail-tab-components"[^>]+tabindex="-1"/)
    assert.match(renderer, /installRovingTablist\(onboardingModes/)
    assert.match(renderer, /installRovingTablist\(detailSections/)
    assert.match(renderer, /nextRovingTabIndex/)
    assert.match(html, /role="status" aria-live="polite"/)
    assert.match(html, /connect-src 'none'/)
    assert.match(html, /Basic inspection/)
    assert.match(html, /Complete inspection/)
    assert.match(html, /id="quick-private-review" class="private-review" hidden/)
    assert.match(html, /id="full-private-review" class="private-review" hidden/)
    assert.match(renderer, /ONBOARDING_ENDPOINT_PRIVATE_REVIEW_REQUIRED/)
    assert.doesNotMatch(renderer, /classifyAddress|isIP|node:dns|dns\/promises/)
    ...
    assert.match(css, /box-shadow: 0 0 0 5px #3c2766/)
    assert.doesNotMatch(css, /focus-visible[^\n]+rgba\(118, 86, 172, 0\.24\)/)
  })
